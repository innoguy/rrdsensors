...
do
  ...
  NET_IN="$(bc -l <<< $(ifstat  -i eth0  1 1 | awk 'FNR==3 {print $1+0}'))"
  NET_OUT="$(bc -l <<< $(ifstat  -i eth0  1 1 | awk 'FNR==3 {print $2+0}'))"
  rrdtool updatev ${OUT}.rrd N:$CPU_LOAD:$CPU_TEMP:$NET_IN:$NET_OUT
  sleep 5
done
